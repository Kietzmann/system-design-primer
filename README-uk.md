...
* Більшість систем "ведучий-ведучий" або слабо узгоджені (порушуючи ACID) або мають збільшені затримки при записі даних через їх синхронізацію.
* Вирішення конфліктів стає тим більшим, чим збільшується кількість вузлів для запису та затримки.
* Розгляньте [Disadvantage(s): replication](#disadvantages-replication) для ідей, пов'язаних з **обома** підходами.

##### Недолік(и): реплікація

* Існує ймовірність втрати даних, якщо ведучий відмовить перед тим, як будь-які нові записані дані можуть бути зрепліковані на інші вузли.
* Записані дані реплікують на репліки зчитування.  Якщо записів багато, репліки зчитування можуть впасти з наступним відтворенням записів і не зможуть осблуговувати багато зчитувань.
* Чим більше існує вузлів для зчитування, тим більше вам необхідно мати реплік, що спричиняє збільшення затримки реплікації.
* На деяких системах записи в ведучі вузли можуть породжувати багаточисленні потоки записувати паралельно, в той час, коли репліки для зчитування підтримують лище послідовні записи, які виконуються одним потоком.
